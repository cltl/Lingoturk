(function () {
    var app = angular.module('TargetIdentificationInContextExperimentApp', ["Lingoturk"]);
...
this.submitResults = function (successCallback, errorCallback) {
    var results = {
        experimentType : "TargetIdentificationInContextExperiment",
    ...
    if(self.questionId != null){
        ...
        $http.get("/returnPart?partId=" + self.partId).success(function (data) {
            ...
            for(var i = 0; i < self.questions.length; ++i){
                ...
                q.comment_religion = q.comment_religion.split(' ');
                q.comment_gender = q.comment_gender.split(' ');
                q.comment_sexual_orientation = q.comment_sexual_orientation.split(' ');
                q.comment_age = q.comment_age.split(' ');
                q.comment_disability = q.comment_disability.split(' ');
                q.comment_individual = q.comment_individual.split(' ');
                q.comment_other= q.comment_other.split(' ');
                q.comment_another= q.comment_another.split(' ');
                q.comment_speaker = q.comment_speaker.split(' ');
                q.comment_race_ethnicity_nationality = q.comment_race_ethnicity_nationality.split(' ');
